...
async function updateCardDetails(instrumentIdentifier: string, paymentInstrument: string, req: Request) {
    try {
        ...
        const isNumericIIN = (iin: string): boolean => /^[0-9]+$/.test(iin || '');
        for (const card of creditCards) {
            if (!isNumericIIN(card.iin)) {
                try {
                    let instrumentIdentifier = card?.cardProps?.instrumentId;
                    let retrievedInstrumentId: PostInstrumentIdentifierRequest = await makeRequest(
                        requestContext.merchantConfig,
                        InstrumentIdentifierApi,
                        "getInstrumentIdentifier",
                        instrumentIdentifier,
                    ...
                    card.iin = retrievedInstrumentId?.card?.number?.slice(0, 6) || card.iin;
                    logger.debug("WebhookRouter tokenUpdate: IIN updated for card");
                } catch (err) {
                    logger.debug("WebhookRouter tokenUpdate: Failed to update IIN for card" + err);
                }
            }
        }
