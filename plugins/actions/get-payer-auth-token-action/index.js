...
function* getPayerAuthTokenActionSaga(action) {
  const response = yield endpointSaga({action, payload: action.payload, endpointId: 'payerAuthEndpoint'});
  return response;
}
...
export default {
  *saga() {
    yield takeEvery('getPayerAuthTokenAction', getPayerAuthTokenActionSaga);
  }
};
